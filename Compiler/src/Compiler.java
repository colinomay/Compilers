...
public class Compiler{
    ...
    public static void main(String args[]) throws Exception {
        ...
        while ((c = br.read()) != -1) {
            character = String.valueOf((char)c).toLowerCase();
        ...
        for(int i=0; i<programs_string.size();i++) {
            ...
            if(errors>0) {
                System.out.println("Parser found errors, stopping Compiler");
                System.exit(0);
            } else {
    ...
    public static ArrayList<Lexeme> parseStatement(ArrayList<Lexeme> program, SyntaxTreeNode node) {
        if (errors == 0) {
            ...
            } else if(Character.isLetter(program.get(0).getValue().charAt(0))) {
    ...
    public static ArrayList<Lexeme> parseVarDecl(ArrayList<Lexeme> program, SyntaxTreeNode node) {
        if(errors==0 && program.size()>0){
            ...
            if (Character.isLetter(program.get(0).getValue().charAt(0))) {
